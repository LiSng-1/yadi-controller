...
# 结果类型定义 - 确保所有返回的帧类型都有对应的显示格式
RESULT_TYPES = {
    'sync': {'format': '同步字节: 0x{{data.value}}'},
    'device_id': {'format': '设备编码: 0x{{data.value}}'},
    'data2': {'format': 'DATA2: 单撑断电={{data.side_stand}}, 启动保护={{data.start_protect}} (原始:{{data.raw}})'},
    'data3': {'format': 'DATA3: 霍尔故障={{data.hall_fault}}, 转把={{data.throttle_fault}}, 控制器={{data.controller_fault}}, 欠压={{data.under_voltage}}, 巡航={{data.cruise}}, 助力={{data.assist}}, 缺相={{data.phase_loss}} ({{data.raw}})'},
    'data4': {'format': 'DATA4: 速度档位={{data.speed_gear}}, 电机运行={{data.motor_running}}, 刹车={{data.brake}}, 滑行充电={{data.coast_charge}}, 防飞车={{data.anti_runaway}} ({{data.raw}})'},
    'data5': {'format': 'DATA5: 70%电流={{data.current_70}}, 一键通={{data.yitong}}, EKK={{data.ekk}}, 过流={{data.over_current}}, 堵转={{data.stall}}, 倒车={{data.reverse}}, 电子刹车={{data.e_brake}}, 限速={{data.speed_limit}} ({{data.raw}})'},
    'data6': {'format': '运行电流: {{data.current}}A ({{data.raw}})'},
    'data7': {'format': '速度高字节: {{data.speed_high}} ({{data.raw}})'},
    'data8': {'format': '速度低字节: {{data.speed_low}} ({{data.raw}})'},
    'data9': {'format': '保留字节9: {{data.reserved}} ({{data.raw}})'},
    'data10': {'format': '保留字节10: {{data.reserved}} ({{data.raw}})'},
    'checksum': {'format': '校验和: 计算值{{data.calculated}} 接收值{{data.received}} {{data.status}}'},
    'frame_complete': {'format': '完整帧: 档位{{data.speed_gear}}, 电流{{data.current}}A, 速度{{data.speed}} (帧计数{{data.frame_count}})'},
...
class Hla(HighLevelAnalyzer):
    ...
    def __init__(self):
        ...
        # 状态机: IDLE, SYNC_RECEIVED, COLLECTING
        self.state = 'IDLE'
        self.frame_buffer: List[int] = []          # 存储当前帧的字节
        self.frame_start_time = None                # 当前帧开始时间
        self.frame_count = 0
        self.error_count = 0
    ...
    def decode(self, frame: AnalyzerFrame) -> Optional[AnalyzerFrame]:
        ...
        frame.type 应为 'data'，frame.data['data'] 包含字节值
        """
        # 只处理数据帧
        if frame.type != 'data':
            return None
        ...
        if isinstance(data, (bytes, bytearray)):
            byte_value = data[0] if len(data) > 0 else None
        else:
            byte_value = data  # 可能是整数
        ...
        if self.state == 'IDLE':
            ...
            # 收集数据字节 (DATA2 ~ DATA11)
            self.frame_buffer.append(byte_value)
            byte_index = len(self.frame_buffer) - 1  # 0-based索引
    ...
    def _parse_data4(self, byte_value: int, frame: AnalyzerFrame) -> AnalyzerFrame:
        ...
        return AnalyzerFrame('data4', frame.start_time, frame.end_time, {
            ...
            # 注意：bit4 是“控制器保护”，但协议中DATA4表格有冲突，这里省略，如有需要可添加
    ...
    def _parse_data5(self, byte_value: int, frame: AnalyzerFrame) -> AnalyzerFrame:
        ...
        return AnalyzerFrame('data5', frame.start_time, frame.end_time, {
            ...
            'yitong': bool((byte_value >> 6) & 0x01),   # 一键通
            'ekk': bool((byte_value >> 5) & 0x01),      # EKK备用电源
    ...
    def _verify_and_complete(self, frame: AnalyzerFrame) -> AnalyzerFrame:
        ...
        if checksum_ok:
            ...
            speed = (self.frame_buffer[7] << 8) | self.frame_buffer[8]
            ...
            # 校验和错误，返回错误帧
            return AnalyzerFrame('checksum', self.frame_start_time, frame.end_time, {
